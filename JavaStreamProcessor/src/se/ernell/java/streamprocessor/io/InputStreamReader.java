...
/*
 * 2013 Robert Andersson <http://www.ernell.se>
 *
...
import se.ernell.java.streamprocessor.processor.IStreamObjectRecievable;

/**
 * This class is derived from Jack Shirazi's fast file reader found in the book
 * 'Java Performance Tuning'.
 */
public class InputStreamReader {

    private static final char MAP3[] = { '\200', '\201', '\202', '\203',
	    ...
	    '\374', '\375', '\376', '\377', 0, '\001', '\002', '\003', '\004',
	    '\005', '\006', '\007', '\b', '\t', '\n', '\013', '\f', '\r',
    ...
    /**
     * min/max-length of a line. all other lines are discarded by the reader
     */
    private static int restricted_min_length = 0;
    private static int restricted_max_length = DEFAULT_MAX_LINE_LENGTH;

    /** reciever object */
    private static IStreamObjectRecievable reciever;

    /** read lines from the stream */
    public final static void readLinesFromStream(InputStream is)
	    throws IOException {
    ...
    public InputStreamReader(IStreamObjectRecievable arg_object) {
	reciever = arg_object;
    }
	...
	reciever.process(line, length);

    }

    /** Print info to console */
    public void printStats() {
	// System.out.println("Chars read from stream: " + tchars
	// + " (linebreaks excluded)");
	// System.out.println("Lines read from stream: " + nlines);
    }

    /**
     * Read lines from the stream. One major flaw with this function is that the
     * last line of the content must be terminated with a linebreak. You also
     * have to know the maximum line length of the content.
     */
    public final static void readLinesFromStream(InputStream is,
	    int maxLineLength, int defaultBufferSize) throws IOException {
	...
	try {
		...
		for (; nextChar < nChars; nextChar++) {
			...
			// Check if line is to be further processed
			if (nLineLength <= restricted_max_length
				&& nLineLength >= restricted_min_length) {
			    // length within range, go on with processing
		...
		if (nStartLineIndex < nChars) {
		    ...
		    System.arraycopy(char_buffer, nStartLineIndex, char_buffer,
			    0, nLeftOver);
		}

		do {
		    // refill the byte buffer from stream
		    length_read = is.read(byte_buffer, nLeftOver,
			    byte_buffer.length - nLeftOver);
		} while (length_read == 0);
		...
		if (length_read > 0) {
		    ...
		    convert(byte_buffer, nLeftOver, nChars, char_buffer,
			    nLeftOver, nChars, MAP3);
    ...
    private static int convert(byte byte_buffer[], int i, int j,
	    char char_buffer[], int k, int l, char MAP_ARRAY[])
	    throws Exception {
	int i1 = j;
	boolean flag = false;
	if (j - i > l - k) {
    ...
    public void setRestrictedMinLength(int restricted_min_length) {
	this.restricted_min_length = restricted_min_length;
    }

    public void setRestrictedMaxLength(int restricted_max_length) {
	this.restricted_max_length = restricted_max_length;
    }

    public void setRestrictedLength(int restricted_min_length,
	    int restricted_max_length) {
	this.restricted_min_length = restricted_min_length;
	this.restricted_max_length = restricted_max_length;
    }
